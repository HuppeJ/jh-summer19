...
import os
try:
	os.chdir(os.path.join(os.getcwd(), 'Exercises\\Exercise6_Concrete_NLP_Tutorial_Adapted\concrete_NLP_tutorial'))
	print(os.getcwd())
except:
	pass
#%% [markdown]
...
with open("socialmedia_relevant_cols.csv", "r", encoding='utf-8', errors='replace') as input_file:
    csv_reader = csv.reader(input_file, delimiter=',')
    with open("socialmedia_relevant_cols_clean.csv", "w", encoding='utf-8', newline='') as output_file:
        csv_writer = csv.writer(output_file, delimiter=',')
        for row in csv_reader:
            out = row
            out[0] = re.sub(r"\s*[^A-Za-z-]+\s*", " ", row[0])
