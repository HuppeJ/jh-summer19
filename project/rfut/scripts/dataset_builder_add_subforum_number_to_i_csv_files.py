...
def run():
    print("Running : run_dataset_builder_add_subforum_number_to_i_csv_files")
    ...
    for subdir, dirs, files in os.walk(posts_directory):
        for file in files:
            ...
            if fileInput.endswith(".csv"):
                ...
                directory = [i for i in list_of_directories if str(i).startswith(str(subforum_num) + "-")][0]
                filename = fileInput
                output_file_path = [DIABETES_DATA_PATH, POSTS_DATA_PATH, directory, filename]
                output_file = os.path.join('', *output_file_path)
                df_input_copy.to_csv(output_file, sep=',', encoding='utf-8', index=False)   
